...
function buildContextBlock({ repo, branch, workspacePath, username }) {
  const lines = [
    ...
    '1. Clone: git_ops(op="clone", url="https://github.com/' + repo + '.git", dest=".", cwd="workspace/{job_id}/", branch="' + branch + '")',
    '   NOTE: git_ops DOES accept the url parameter directly — do NOT use run_bash for git clone.',
    '2. New branch (REQUIRED naming): git_ops(op="branch", action="create", name="DeeperSeek/<short-task-slug>", cwd="...")',
    '   NEVER commit directly to main/master. ALWAYS use DeeperSeek/ prefix for branch names.',
    '3. Stage files: git_ops(op="add", paths=["file1", "file2"], cwd="...")',
    '4. Commit: git_ops(op="commit", message="feat: describe change", cwd="...")',
    '   Commit identity is injected automatically — do NOT set user.name/email manually.',
    '5. Push: git_ops(op="push", cwd="...") — GITHUB_TOKEN auth is automatic.',
    '6. PR / issues / file API: use github_ops (op=create_pr, list_prs, get_file, etc.)',
